...
class Game extends EventEmitter {
  ...
  hasPlayerWithName(playerName) {
    return !!this._getPlayerWithName(playerName);
  }
  ...
  _playCardAsPlayer(playerName, cardName, targetPlayerName, cardChoice) {
    if (!this._started) {
      this._events.createGameNotStartedError(playerName);
      return;
    }

    const player = this._getPlayerWithName(playerName);
    ...
    if (targetPlayerName) {
      targetPlayer = this._getPlayerWithName(targetPlayerName);

      if (!targetPlayer) {
        this._events.createNoTargetPlayerError(playerName, targetPlayerName);
        return;
      }
    ...
    const card = player.getHandCardByName(cardName);

    if (!card) {
      this._events.createPlayerHasNoCardError(playerName, cardName);
      return;
    }
    ...
    if (validTargets.length > 0 && !targetInValidTargets) {
      ...
      this._events.createInvalidCardTargetError(
        playerName, cardName, targetPlayer
      );
      return;
    }
    ...
    if (targetPlayerName && (card instanceof Guard)) {
      ...
      if (typeof cardChoice === 'string') {
        const card = this._deck.getAllCards()
          .filter(c => !(c instanceof Guard))
          .find(c => c.getName().toLowerCase() === cardChoice.toLowerCase());

        if (card) {
          isValidChoice = true;
          cardChoice = card.getName();
        }
      }
