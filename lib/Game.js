...
class Game extends EventEmitter {
  ...
  startGameAsPlayer(playerName) {
    ...
    const randomSeed = Random.engines.mt19937().autoSeed()();
    this._events.createStartInput(playerName, randomSeed);
  }
  ...
  _delegateInput(event) {
    switch (event.type) {
      case 'playerJoin':
        ...
        break;
      case 'gameStart':
        this._startGameAsPlayer(event.playerName, event.randomSeed);
        break;
      case 'cardPlay':
  ...
  _startGameAsPlayer(playerName, randomSeed) {
    if (this.hasStarted()) {
    ...
    this._random = new Random(Random.engines.mt19937().seed(randomSeed));

    // Randomize starting player
    this._random.shuffle(this._players);
  ...
  _startNewRound() {
    this._deck = new Deck(this._random);
    this._deck.shuffle();
